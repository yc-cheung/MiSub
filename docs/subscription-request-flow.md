...
节点列表生成后，输出由 `renderResponse({...})` 阶段统一处理（接收已解析的请求上下文 + 节点列表，返回 Response），根据目标格式和引擎进入不同输出分支。各分支的响应头/响应体保持与重构前一致；访问通知与访问日志由各分支共用的 `notifyAndLogAccess()` 负责（按通知标签、是否受 `enableAccessLog` 门控、写日志用的格式串三个参数区分）。
...
通知与访问日志由 `notifyAndLogAccess()` 统一处理（三个输出分支共用，原先是近乎复制粘贴的三段）。访问通知与访问日志不是完全同一个开关。
