...
functions/modules/handlers/telegram-webhook-handler.js  # Telegram Bot 入口：校验+权限+命令路由表（装配层，~160 行）
functions/modules/handlers/telegram-transport.js        # Telegram send/edit/answer 传输适配器（可在测试中注入 fake）
functions/modules/handlers/telegram/shared.js           # 存储/配置/传输/权限/限流/用户节点等共享工具
functions/modules/handlers/telegram/formatters.js       # 纯消息构建函数（可快照测试）
functions/modules/handlers/telegram/commands.js         # 各命令处理器（/start /list /enable ...）
functions/modules/handlers/telegram/callbacks.js        # 按钮回调分发（CALLBACK_ACTIONS 精确表 + CALLBACK_PREFIXES 前缀表）
