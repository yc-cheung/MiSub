/**
 * Telegram Bot 命令处理器（/start /help /list /enable ... 等）。
 * 由 ../telegram-webhook-handler.js 的命令路由表按命令分发；逐字从原 handler 迁出，行为不变。
 */
...
import {
    getStorageAdapter, getCachedStorageAdapter, createRequestCache,
    getCachedSettings, getCachedSubscriptions, getCachedProfiles,
    persistCachedSubscriptions, persistCachedProfiles, persistCachedSettings,
    getTelegramPushConfig, generateId, extractNodeName, extractNodeUrls, parseTargetArgs,
    sendTelegramMessage, editTelegramMessage, answerCallbackQuery,
    getUserBoundProfileId, setUserBoundProfileId, checkUserPermission, checkRateLimit,
    getUserNodes, getNodesWithMapping
} from './shared.js';
import { formatStartMessage, formatHelpMessage } from './formatters.js';

export async function handleStartCommand(chatId, env) {
    await sendTelegramMessage(chatId, formatStartMessage(), env);
}
...
export async function handleHelpCommand(chatId, env) {
    await sendTelegramMessage(chatId, formatHelpMessage(), env);
}

/**
 * 处理 /menu 命令 - 快捷菜单
 */
export async function handleMenuCommand(chatId, env, messageId = null, requestCache = null) {
    const keyboard = {
        inline_keyboard: [
            [
 ...
 */
export async function handleListCommand(chatId, userId, env, page = 0, type = 'all', messageId = null, requestCache = null) {
    try {
 ...
 * 处理 /stats 命令
 */
export async function handleStatsCommand(chatId, userId, env, requestCache = null) {
    try {
 ...
 * 处理 /delete 命令
 */
export async function handleDeleteCommand(chatId, userId, args, env) {
    try {
 ...
 * 处理 /enable 命令
 */
export async function handleEnableCommand(chatId, userId, args, env) {
 ...
 * 处理 /disable 命令
 */
export async function handleDisableCommand(chatId, userId, args, env) {
 ...
 * 切换节点启用状态
 */
export async function handleToggleCommand(chatId, userId, args, env, enable) {
    try {
        const target = parseTargetArgs(args);
 ...
 * 处理 /search 命令
 */
export async function handleSearchCommand(chatId, userId, args, env) {
    try {
 ...
 * 处理 /sub 命令 - 获取订阅链接
 */
export async function handleSubCommand(chatId, args, env, request, requestCache = null) {
    try {
        const cache = requestCache || createRequestCache();
        const profiles = await getCachedProfiles(env, cache);
        const settings = await getCachedSettings(env, cache);
 ...
 */
export async function handleSubCommandSimple(chatId, env, requestCache = null) {
    try {
        const cache = requestCache || createRequestCache();
        const profiles = await getCachedProfiles(env, cache);
        const settings = await getCachedSettings(env, cache);
 ...
 * 处理 /rename 命令
 */
export async function handleRenameCommand(chatId, userId, args, env) {
    try {
        if (args.length < 2) {
            await sendTelegramMessage(chatId,
                '✏️ <b>重命名节点</b>\n\n' +
 ...
 * 处理 /info 命令 - 节点详情
 */
export async function handleInfoCommand(chatId, userId, args, env) {
    try {
        if (args.length === 0) {
            await sendTelegramMessage(chatId,
                '📄 <b>查看节点详情</b>\n\n' +
 ...
 * 处理 /copy 命令 - 复制节点链接
 */
export async function handleCopyCommand(chatId, userId, args, env) {
    try {
        const target = parseTargetArgs(args);
 ...
 * 处理 /export 命令 - 导出节点
 */
export async function handleExportCommand(chatId, userId, args, env) {
    try {
        const userNodes = await getUserNodes(userId, env);
 ...
 * 处理 /import 命令 - 导入节点
 */
export async function handleImportCommand(chatId, userId, args, env) {
    try {
        if (args.length === 0) {
            await sendTelegramMessage(chatId,
                '📥 <b>导入节点</b>\n\n' +
 ...
 * 处理 /sort 命令 - 节点排序
 */
export async function handleSortCommand(chatId, userId, args, env) {
    try {
 ...
 * 处理 /dup 命令 - 去重检测
 */
export async function handleDupCommand(chatId, userId, args, env) {
    try {
 ...
 * 处理 /bind 命令 - 绑定默认订阅组
 */
export async function handleBindCommand(chatId, userId, args, env, requestCache = null) {
    try {
        const cache = requestCache || createRequestCache();
        const profiles = await getCachedProfiles(env, cache);
        const settings = await getCachedSettings(env, cache);
 ...
 * 处理 /unbind 命令 - 解除绑定
 */
export async function handleUnbindCommand(chatId, userId, env, requestCache = null) {
    try {
        const cache = requestCache || createRequestCache();
        const settings = await getCachedSettings(env, cache);
        const config = settings.telegram_push_config || {};
 ...
 * 处理节点输入（核心逻辑）
 */
export async function handleNodeInput(chatId, text, userId, env, requestCache = null) {
    try {
        const cache = requestCache || createRequestCache();
